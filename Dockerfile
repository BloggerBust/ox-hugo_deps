...
COPY --from=go /usr/local/bin/ /usr/local/bingo
...
# thrills :-P
RUN cp -run /usr/local/bingo/* /usr/local/bin/; \
    cp -run /usr/local/libgo/* /usr/local/lib/; \
    rm -rf /usr/local/bingo/ && rm -rf /usr/local/libgo/; \
